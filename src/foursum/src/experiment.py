# this is python3
import sys,os,random,subprocess,statistics,pathlib
...
simpPyth=(python, 'pythonSol/foursum_n4.py')
fastPyth=(python, 'pythonSol/foursum_n3.py')
dictPyth=(python, 'pythonSol/foursum_n2.py')
...
def prodExp(prod,name,extra=[]):
    ...
    runExp(prod,simpPyth,tableFile='Tables/'+name+'PythSimp.table', Nlist=llNlist,extra=extra)
...
def runExp(producer,foursum,tableFile='', Nlist=[100], extra=[]):
    ...
    for i in range(4):
      for N in Nlist:
        ...
        try: 
            output = subprocess.check_output(foursum, stdin=ps.stdout,stderr=subprocess.DEVNULL, timeout = 5)
